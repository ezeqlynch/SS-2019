...
if __name__ == "__main__":
    ...
    print(avgEndTimesErr)
    plt.errorbar(list(map(lambda d: float(d), parsedArgs.d)), avgEndTimes, avgEndTimesErr, marker=".", 
    markersize=5, linewidth=0.5, elinewidth= 2, label="Tiempo promedio de salida en " + str(parsedArgs.simus) + " corridas")
    # plt.plot(times, avgKE, marker=".", markersize=3,
    #             linewidth=0.5, label="Promedio KE d=0," + d, yerr=avgKEErr)

    # plt.axis([0, 5, -1, 1])
    plt.ylabel('Tiempo (s)')
    plt.xlabel('Velocidad Deseada, Vmax (m/s)')
    # plt.yscale("log")
    # plt.xscale("log")
    plt.legend(loc='best')
    plt.ylim(ymin=0)

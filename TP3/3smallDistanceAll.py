...
if __name__ == "__main__":
    ...
    for file in os.listdir(parsedArgs.staticFile):
        if file.endswith(".stat"):
            ...
            staticFile.readline()
            staticFile.readline()
            initP = staticFile.readline().split(' ')
            for i in range(particleNum-2):
                staticFile.readline()
            staticFile.readline()
            ...
            for i in range(collisionNum):
                ...
                pos = staticFile.readline().split(' ') # part chica
                if (float(nt) > last):
                    y.append((float(initP[1]) - float(pos[1])) ** 2 + (float(initP[2]) - float(pos[2])) ** 2)
                    x.append(float(nt))
                    last += deltaTime
                # x.append(float(pos[1]))
                # y.append(float(pos[2]))
                coll1 = staticFile.readline().split(' ')  # choque 1
                coll2 = staticFile.readline().split(' ')  # choque 2
                if(int(coll1[0]) == 1 and int(coll2[0]) < 0):
                    cut = True
                    break
    ...
    auxTimes2= np.arange(0, 10 * len(allX[0]), 0.01)
    ...
    plt.ylabel('DCM(m^2)')
    plt.xlabel('Tiempo(s)')
    plt.title('Desplazamiento Cuadrtico Medio')
